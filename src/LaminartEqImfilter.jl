...
function I_u!(r::AbstractArray, I::AbstractArray, p::NamedTuple)
    imfilter!(r, I, p.k_gauss_1, Fill(0f0))
    @. r = I - r
    return nothing
end
...
function func_filter_W!(
    ...
    for k ∈ 1:p.K
        ...
        img_k = @view img[:, :, k]
        out_k = @view W_out[:, :, k]
        imfilter!(out_k, img_k, centered(W[:, :, k, k]), Fill(0f0))
        for l ∈ 1:p.K
            if l ≠ k
                img_l = @view img[:, :, l]
                imfilter!(W_temp, img_l, centered(W[:, :, k, l]), Fill(0f0))
                @. out_k += W_temp
            end
        end
    end
...
function fun_dv!(
    ...
    imfilter!(dv, x_lgn, centered(p.k_gauss_1), Fill(0f0))
    @. dv =
        p.δ_v * (
...
function fun_v_C!(
    ...
    imfilter!(v_C_temp1, v_C_temp2, centered(p.k_gauss_2), Fill(0f0))
    ...
    for k ∈ 1:p.K
        a = @view v_C_tempA[:, :, k]
        b = @view v_C[:, :, k]
        imfilter!(a, v_C_temp1, centered(p.k_C_A[:, :, k]), Fill(0f0))
        imfilter!(b, v_C_temp1, centered(p.k_C_B[:, :, k]), Fill(0f0))
    end
    # end
    @. v_C =
...
function fun_dz!(
    ...
    imfilter!(dz, s, centered(p.k_T_p), Fill(0f0))
    @. dz =
        p.δ_z * (
            -z +
...
function fun_ds!(
    ...
    imfilter!(ds, s, centered(p.k_T_m), Fill(0f0))
...
function fun_H_z!(
    ...
    for k ∈ 1:p.K
        ...
        imfilter!(H_z_k, temp_k, centered(p.k_H[:, :, k]), Fill(0f0))
    end
    # end
    return nothing
end
...
function fun_y_equ!(
    ...
    # 	@inbounds begin
    imfilter!(dy_temp, m, p.k_W_p, Fill(0f0))
    @. dy_temp = m * dy_temp
    fun_f!(dy_temp, p)
