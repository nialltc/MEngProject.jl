...
function func_filter_W!(
    ...
    for k ∈ 1:p.K
        ...
        img_k = @view img[:, :, k]
        out_k = @view W_out[:, :, k]
        imfilter!(out_k, img_k, centered(W[:, :, k, k]), )
        for l ∈ 1:p.K
            if l ≠ k
                img_l = @view img[:, :, l]
                imfilter!(W_temp, img_l, centered(W[:, :, k, l]), )
                @. out_k += W_temp
            end
        end
    end
...
function fun_dv!(
    ...
    imfilter!(dv, x_lgn, centered(p.k_gauss_1), )
    @. dv =
        p.δ_v * (
...
function fun_v_C!(
    ...
    for k ∈ 1:p.K
        a = @view v_C_tempA[:, :, k]
        b = @view v_C[:, :, k]
        imfilter!(a, v_C_temp1, centered(p.k_C_d[:, :, k]), )
...
function fun_dz!(
    ...
    imfilter!(dz, s, centered(p.k_T_p), )
    @. dz =
        p.δ_z * (
            -z +
...
function fun_ds!(
    ...
    imfilter!(ds, s, centered(p.k_T_m), )
...
function fun_H_z!(
    ...
    for k ∈ 1:p.K
        ...
        imfilter!(H_z_k, temp_k, centered(p.k_H[:, :, k]), )
    end
    # end
    return nothing
end
