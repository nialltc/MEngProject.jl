	...
	# 				for t ∈ [25,50,100]
				...
				for k ∈ 1:2:10
					...
					layer=Utils.layers[k]
						...
						plt.axis("off")
						fig.tight_layout()
					plt.savefig(plotsdir(string("batch",batch_),string(file,"_noise_",noise,"_t",t,"_",Utils.la[k],".png")))
					close("all")
				end
		...
		# time plot
			...
			plt.legend()
			fig.tight_layout()
			plt.savefig(plotsdir(string("batch",batch_),string(file,"_noise_",noise,"_time.png")))
			close("all")
		end
	end
end