...
@inbounds begin
	...
	for file in files[1:end]
		...
		for para_test ∈ enumerate(para_sets)
			...
			p = LaminartInitFunc.parameterInit_conv_gpu(datadir("img",file), Parameters.para_var(para_test[2]));
			...
			k = 7
					...
					layer=Utils.layers[k]
						plt.title(string("Layer: $layer, \$t=$t\$, ", test_name_plt[para_test[1]]))
						plt.axis("off")
						fig.tight_layout()
					plt.savefig(plotsdir(string("bench_kern",batch_),string(file,"_para_",test_name[para_test[1]],"_t",t,"_",Utils.la[k],".png")))
					close("all")
		...
		# time plot
			...
			plt.legend()
			fig.tight_layout()
			plt.savefig(plotsdir(string("bench_kern",batch_),string(file,"_para_",test_name[para_test[1]],"_time.png")))
			close("all")
		end
		...
		for test ∈ 1:para_test[1]
			ax.scatter(median(benchm[para_test[1]].times), benchm[para_test[1]].memory, label=test_name_plt,
